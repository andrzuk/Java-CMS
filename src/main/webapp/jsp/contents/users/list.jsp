...
<table align="center" width="95%" cellpadding="5" cellspacing="0">
	...
	<c:forEach items="${data}" var="user" varStatus="iterator">
		...
		<tr class="${iterator.index % 2 == 0 ? 'even' : 'odd'} <c:out value="${style_name}" />">
			...
			<td style="text-align: <c:out value="${sorting.fields_aligns[1]}" />;"><c:out value="${user.first_name}" /></td>
			<td style="text-align: <c:out value="${sorting.fields_aligns[2]}" />;"><c:out value="${user.last_name}" /></td>
			<td style="text-align: <c:out value="${sorting.fields_aligns[3]}" />;"><c:out value="${user.email}" /></td>
			<td style="text-align: <c:out value="${sorting.fields_aligns[4]}" />;"><c:out value="${user.login}" /></td>
			<td style="text-align: <c:out value="${sorting.fields_aligns[5]}" />;"><c:out value="${status_name}" /></td>
			<td style="text-align: <c:out value="${sorting.fields_aligns[6]}" />;"><c:out value="${user.modified_short}" /></td>
			...
			<td class="Action"><a href="/<c:out value="${page.module}" />?action=password&id=<c:out value="${user.id}"/>"><img alt="edit" src="images/access.png" title="Zmień hasło"></a></td>
			<td class="Action"><a href="/<c:out value="${page.module}" />?action=delete&id=<c:out value="${user.id}"/>"><img alt="delete" src="images/trash.png" title="Usuń"></a></td>
		</tr>
	</c:forEach>
